...
export default {
  ...
  props: {
    contents: { type: [String, Object, null], default: null },
  },
  watch: {
    contents(newContent) {
      if (newContent) {
        ...
        this.$nextTick(() => {
          const active = this.$refs.contents.getElementsByClassName('is-active');
          if (active.length == 0) return;
          let scrollTo = active.length * -24;
          for (let i = 0; i < active.length; i++) scrollTo += active[i].offsetTop;
  ...
  methods: {
    close() { this.$store.commit('closeModal'); },
  },
};
</script>
